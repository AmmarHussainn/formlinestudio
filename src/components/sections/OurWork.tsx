            ...
            return (
              <button
                ...
                className={`w-full block transition-all duration-700 ease-in-out border-b border-white/10 last:border-b-0 cursor-pointer outline-none focus-visible:ring-2 focus-visible:ring-white/20 text-left overflow-hidden ${
                  isActive
                    ? "bg-[#DF3B01] h-full md:h-[460px] lg:h-[490px] xl:h-[550px]"
                    : "bg-[#220301] h-[140px] md:h-[120px] lg:h-[200px] xl:h-[250px]"
                }`}
                onClick={() => setActiveIndex(index)}
                data-glow-area="true"
              >
                <div className="flex w-full flex-col md:flex-row items-center md:items-start justify-between p-8 md:px-12 md:py-0 relative h-full">
                  {/* Left Number Index */}
                  <div
                    className={`lg:w-[50%] md:w-[35%] w-full font-orbitron font-black leading-none transition-all duration-500 select-none text-white ${
                      isActive ? "opacity-100" : "opacity-20"
                    } text-[170px] lg:text-[270px] xl:text-[360px]`}
                  >
                    {item.id}
                  </div>

                  {/* Right Side Content */}
                  <div className="w-full lg:w-[50%] md:w-[60%] text-left md:pt-12 lg:pt-20 z-10 md:pl-12">
                    ...
                    >
                      <p className="text-white font-medium text-sm md:text-base lg:text-[16px] leading-[1.6] max-w-xl">
                        {item.description}
                      </p>
                    </div>
                  </div>
